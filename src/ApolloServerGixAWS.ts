...
export interface GixAWSApolloCreateHandlerOptions {
    cors?: {
    ...
    onHealthCheck?: (req: GixAPIGatewayProxyEvent) => Promise<any>;
}
...
export class ApolloServerGixAWS extends ApolloServerBase {
    ...
    createGraphQLServerOptions(
        event: GixAPIGatewayProxyEvent,
        context: LambdaContext,
    ): Promise<GraphQLOptions> {
        return super.graphQLServerOptions({ event, context });
    }

    public createHandler({ cors, onHealthCheck }: GixAWSApolloCreateHandlerOptions = { cors: undefined, onHealthCheck: undefined }) {
        ...
        return (
            ...
            event.path = event.body.path;
            event.httpMethod = event.body.httpMethod;
            event.headers = event.body.headers;
            const eventHeaders = new Headers(event.body.headers);
            ...
            const fileUploadHandler = (next: Function) => {
                const contentType = (
                    event.body.headers['content-type'] || event.body.headers['Content-Type'] || ''
                ).toLowerCase();
